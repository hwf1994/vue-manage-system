...
1.首页设计排版的问题(抽奖区域删掉),头部天气去掉,分享按钮去掉把定位显示放上去
2.首页显示课程的问题目前首页上展示的上某一个机构的展示课程具体不清楚是那个机构的(功能需求上)
3.首页商户入驻的按钮 点击后应该做一个页面进行某机构的信息填写以及图片上传功能到达我们后台通过审核的方式进行处理
4.授权登录 缺少授权登录页面 或者弹框进行授权登录 流程应改为先授权登录后进行手机绑定
5.我的-->全部课程里面不清晰具体的帅选类别干嘛用的 
6.简化部分功能点 银行卡绑定,学币等功能
7.机构详情里面图片保存方式处理
8.搜索框 热搜带滚动条处理
9.评论管理 订单管理 地理位置编辑   (活动模块先隐藏掉)